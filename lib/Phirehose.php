...
abstract class Phirehose
{
  ...
  * @todo Shouldn't really hard-code URL strings in this function.
  ...
  public function setLocations($boundingBoxes)
  {
    ...
    foreach ($boundingBoxes as $boundingBox) {
      ...
      if (count($boundingBox) != 4) {
        ...
        logEntry('Invalid location bounding box: [' . implode(', ', $boundingBox) . ']','error');
        return FALSE;
      }
  ...
  public function setLocationsByCircle($locations) {
    ...
    foreach ($locations as $locTriplet) {
      ...
      if (count($locTriplet) != 3) {
        // Invalid - Not much we can do here but log error
        logEntry('Invalid location triplet for ' . __METHOD__ . ': [' . implode(', ', $locTriplet) . ']','error');
        return FALSE;
      }
      ...
      // Debugging is handy
      logEntry('Resolved location circle [' . $lon . ', ' . $lat . ', r: ' . $radius . '] -> bbox: [' . $minLon .
    ...
    do {
      ...
      while ($this->conn !== NULL && !feof($this->conn) &&
        ...
        if ((time() - $lastStreamActivity) > $this->idleReconnectTimeout) {
          logEntry('Idle timeout: No stream activity for > ' . $this->idleReconnectTimeout . ' seconds. ' . 
           ' Reconnecting.','info');
          $this->reconnect();
        ...
        if ($this->filterChanged == TRUE && (time() - $lastFilterUpd) >= $this->filterUpdMin) {
          logEntry('Reconnecting due to changed filter predicates.','info');
          $this->reconnect();
          $lastFilterUpd = time();
        }
      ...
      logEntry('Phirehose connection error occured: ' . $this->lastErrorMsg,'error');
  ...
  {
      logEntry('Consume rate: ' . $this->statusRate . ' status/sec (' . $this->statusCount . ' total), avg ' . 
  ...
  protected function connect()
  {
    ...
    do {
      ...
      // Debugging is useful
      logEntry('Connecting to twitter stream: ' . $url . ' with params: ' . str_replace("\n", '',
      ...
      logEntry("Connecting to {$scheme}{$urlParts['host']}, port={$port}, connectTimeout={$this->connectTimeout}");
      ...
      if (!$this->conn || !is_resource($this->conn)) {
        ...
        logEntry('TCP failure ' . $connectFailures . ' of ' . $this->connectFailuresMax . ' connecting to stream: ' .
      ...
      logEntry('Connection established to ' . $urlParts['host']);
      ...
      if ($httpCode != 200) {
        ...
        logEntry('HTTP failure ' . $connectFailures . ' of ' . $this->connectFailuresMax . ' connecting to stream: ' . 
  ...
  protected function disconnect()
  {
    if (is_resource($this->conn)) {
      logEntry('Closing Phirehose connection.');
      fclose($this->conn);
    }
    $this->conn = NULL;
